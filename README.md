...
42) Answer: 1) Which type of tag would you not find in the document head? <p>  2) What are the six major sectioning elements that typically go directly in the body element? <aside>, <main>, <article>, <header>, <footer>, <section> 3) What is the difference between a head element and a header element? The head element contains document metadata, and a header element contains text to be displayed  4) Which section of an HTML page should contain metadata? The head

43) Forms: Use the input element to provide places for users to input their name and email. Unlike other elements, the input element does not have a closing tag due to its older structure. It acts as a marker for the browser to bring in functionality and place it there. To make it accessible to everyone, we need to address the issue of the label and input elements not being connected. There are two options to achieve this. Add a "for" attribute to the label that matches the "id" attribute of the input. Wrap the input with the label.

44) For the email field, tell the browser to collect an email address, and instruct the browser to help the user fill out the form correctly. Type equals email. Now when people fill out this form, the browser will verify that the data that was entered is an email. When a user tries to type anything other than an email address, they get a warning, and are asked to fix it.

45) Placeholder:  include a placeholder, or default value. Many times, designers want a form field to be pre-populated with a suggestion of what should go into that field. It could help users understand what they should do. HTML can put a placeholder in the form. Use the placeholder attribute, and put a suggestion, or an example in the field intended. The suggested text should appear after this. The suggested text will appear light gray by default and as soon as this field is clicked, the placeholder text disappears. This is important. Users should not have to erase this placeholder text in order to write their own email address. It purely serves as a suggestion and when no longer required, it should disappear.
46) add a value attribute. This pre-populates the field with real content. When an email address is added, the suggested one is in the way. This has to be manually erased to place the actual email address.
47) The value attribute is for including such data in the form. If both are included into the example, and the form is submitted to compare the results, the information that was displayed by placeholder is not submitted as real data. The information that was pre-populated by value is. 
